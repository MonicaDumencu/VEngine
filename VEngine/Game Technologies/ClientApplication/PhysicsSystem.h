...
public:
	...
	// -------- Added by Monica ------------- //
	void	SetPlayerEntity(PlayerClass* ent) { player = ent; }
	void	SetPlayerPhysicsNode(PhysicsNode* pNode) { playerPhysics = pNode; }
...
//Statics
	...
	//use this vector below to avoid iteration error.
	vector<PhysicsNode*> copiedNodes;
	//use this mutex each time we add or remove a physics node
	mutex allNodesMutex;
};