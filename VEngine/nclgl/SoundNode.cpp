...
void		SoundNode::AttachSource(OALSource* s) {
	...
	if(timeLeft > 0) {
		...
		alSourcef(oalSource->source,AL_SEC_OFFSET,(sound->length/ 1000.0) - (timeLeft / 1000.0));

			cout << "Attaching! Timeleft: " << (sound->length/ 1000.0) - (timeLeft / 1000.0) << endl;
