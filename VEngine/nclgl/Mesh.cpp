...
Mesh* Mesh::GenerateQuad()	{
	...
	//use indices to make normal of the quad right. modified by Kan
	m->indices[0] = 0;
	m->indices[1] = 1;
	m->indices[2] = 2;
	m->indices[3] = 2;
...
void	Mesh::BufferData()	{
	...
	if (binormals)
	{
		glGenBuffers(1, &bufferObject[BINORMAL_BUFFER]);
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[BINORMAL_BUFFER]);
		glBufferData(GL_ARRAY_BUFFER, numVertices*sizeof(Vector3), binormals, GL_STATIC_DRAW);
		glVertexAttribPointer(BINORMAL_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(BINORMAL_BUFFER);
	}
	//Buffer tangent data
	if(tangents) {
...
/*
Author: Monica
Purpose: Create a cube for the voxel engine
Generate a cube mesh. No indices because it is more difficult to generate texture coordinates for indexed cubes.
(Tried it for the graphics corsework and was unable to make it look good).
*/
...
/*
Author: Monica
Purpose: Add new vertices to the mesh as it grows, and texture coordinates. Do not buffer them yet.
The buffering will take place at the end, when the entire mesh has been created.
*/
void Mesh::AddCubeToMesh(Vector3* arrayVertices, Vector2* arrayTextures,Vector3* arrayNormals, Vector3* arrayTangents, int nrVert) {
	//copy the new data in newVertices
	//i is the last index added into the newVertices list
	for (int j = 0; j < nrVert; j++) {
		vertices[numVertices + j] = arrayVertices[j];
	}

	//do the same with textures
	//we already updated the number of vertices
	for (int j = 0; j < nrVert; j++) {
		textureCoords[numVertices + j] = arrayTextures[j];
	}

	for (int j = 0; j < nrVert; j++) {
		normals[numVertices + j] = arrayNormals[j];
	}

	for (int j = 0; j < nrVert; j++) {
		tangents[numVertices + j] = arrayTangents[j];
	}

	//update the vertices, colors and number of vertices of the last mesh
	numVertices = (numVertices + nrVert);
}

void Mesh::UnmapBufferData() {
	glUnmapBuffer(GL_ARRAY_BUFFER);
	glBindVertexArray(0);
}

void* Mesh::GetMapBuffer(MeshBuffer buffOb, float size) {
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[buffOb]);
	return glMapBufferRange(GL_ARRAY_BUFFER, 0, this->numVertices * size, GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
}

void Mesh::UpdateMeshData() {
	ptrVertex = (Vector3*)GetMapBuffer(VERTEX_BUFFER, sizeof(Vector3));
	memcpy(ptrVertex, this->vertices, this->numVertices * sizeof(Vector3));
	UnmapBufferData();

	ptrTexCoord = (Vector2*)GetMapBuffer(TEXTURE_BUFFER, sizeof(Vector2));
	memcpy(ptrTexCoord, this->textureCoords, this->numVertices * sizeof(Vector2));
	UnmapBufferData();

	ptrNormals = (Vector3*)GetMapBuffer(NORMAL_BUFFER, sizeof(Vector3));
	if (ptrNormals) {
		memcpy(ptrNormals, this->normals, this->numVertices * sizeof(Vector3));
		UnmapBufferData();
	}

	ptrTangents = (Vector3*)GetMapBuffer(TANGENT_BUFFER, sizeof(Vector3));
	if (ptrTangents) {
	memcpy(ptrTangents, this->tangents, this->numVertices * sizeof(Vector3));
	UnmapBufferData();
	}
}

void Mesh::BindBuffers(const int nrVert) {

	vertices = new Vector3[nrVert];
	textureCoords = new Vector2[nrVert];
	normals = new Vector3[nrVert];
	tangents = new Vector3[nrVert];
	...
	glGenBuffers(1, &bufferObject[VERTEX_BUFFER]);
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[VERTEX_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER, nrVert * sizeof(Vector3), this->vertices, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(VERTEX_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(VERTEX_BUFFER);
	...
	glGenBuffers(1, &bufferObject[TEXTURE_BUFFER]);
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER, nrVert * sizeof(Vector2), this->textureCoords, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(TEXTURE_BUFFER, 2, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(TEXTURE_BUFFER);
	...
	glGenBuffers(1, &bufferObject[NORMAL_BUFFER]);
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[NORMAL_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER, nrVert * sizeof(Vector3), this->normals, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(NORMAL_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(NORMAL_BUFFER);
	...
	glGenBuffers(1, &bufferObject[TANGENT_BUFFER]);
	glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TANGENT_BUFFER]);
	glBufferData(GL_ARRAY_BUFFER, nrVert * sizeof(Vector3), this->tangents, GL_DYNAMIC_DRAW);
	glVertexAttribPointer(TANGENT_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(TANGENT_BUFFER);
...
void	Mesh::GenerateNormalsTerrain()	{
	delete[] normals;
	normals = new Vector3[numVertices];
...
void Mesh::GenerateTangentsTerrain() {
	//Extra! stops rare occurrence of this function being called
	//on a mesh without tex coords, which would break quite badly!
	if (!textureCoords) {
		return;
